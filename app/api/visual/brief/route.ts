...
import { AiError, visualBrief } from "@/lib/ai";
    ...
    const safeHistory: ConsultTurn[] = Array.isArray(history)
      ? history.filter(
          (t: unknown): t is ConsultTurn =>
            !!t && typeof t === "object" && typeof (t as ConsultTurn).text === "string",
        )
      : [];
    const result = await visualBrief(safeHistory, text, typeof lang === "string" ? lang : undefined);
